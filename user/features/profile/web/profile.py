...
async def me(user: UserRead = Depends(admin_auth_service.get_user)):
    return user


@router.get("/license", response_model=LicenseRead)
async def get_license(
    session: DBSession, account_uuid: str, user: UserRead = Depends(admin_auth_service.get_user)
):
...
async def change_profile_name(
    *,
    session: DBSession,
    user: UserRead = Depends(admin_auth_service.get_user),
    name: str,
):
...
@router.get("/ping")
async def ping(session: DBSession, user: UserRead = Depends(admin_auth_service.get_user)):
