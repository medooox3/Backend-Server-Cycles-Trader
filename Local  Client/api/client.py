...
class client():
    def __init__(self, event_handler=None, metatrader_dir_path='',
        ...
        self.path_orders = join(metatrader_dir_path,
                                'YM', '_Orders.txt')
        self.path_subscribed_symbols = join(metatrader_dir_path,    
                                            'YM', '_Subscribed_Symbols.txt')
        
        self.path_tabs = join(metatrader_dir_path,'YM', '_Tabs.txt')
    ...
    def open_order(self, symbol='EURUSD',
        ...
        data = [symbol, order_type, lots, price, stop_loss,
                take_profit, magic, comment, expiration,index]
    ...
    def close_orders_by_magic(self, magic):
        """
        Closes orders based on the given magic number.
        
        Args:
            magic (int): The magic number to filter orders.
            
        Returns:
            None
        """
        self.send_command('CLOSE_ORDERS_BY_MAGIC', magic)
        print("close_orders_by_magic")
    ...
    def send_command(self, command, content):
                ...
                if not exists(file_path):
                    try:
                        with open(file_path, 'w') as f:
                            ...
                            print(f'Wrote command {command} to file {file_path}')
                        success = True
                        break
                    except:
                        print_exc()
        ...
        # release lock again
        self.lock.release()
    # function store subribed symbols in a file to be read by server when server restarts or new client connects to server
    def get_subscribed_symbols(self):
        ...
        Returns:
            [type]: [description]
        """
        text = self.try_read_file(self.path_subscribed_symbols)
        data = json.loads(text)
        return data
        
    def set_subscribed_symbols(self, symbol):
        """AI is creating summary for set_subscribed_symbols

        Args:
            symbol ([type]): [description]
        """
        # add symbol to dictionary of subscribed symbols file if not already in list of subscribed symbols
        if not exists(self.path_subscribed_symbols):
                    try:
                        with open(self.path_subscribed_symbols, 'w') as f:
                            #write symbol to dictionary of subscribed symbols file
                            f.write(f'{{"{symbol}"}}')
                    except:
                        print_exc()
        else:
            text = self.try_read_file(self.path_subscribed_symbols)
            # check if symbol is already in list of subscribed symbols
            if symbol not in text:
                # add symbol to dictionary of subscribed symbols file
                try:
                    with open(self.path_subscribed_symbols, 'w') as f:
                        #write symbol to dictionary of subscribed symbols file
                        f.write(f'{text[:-1]},"{symbol}"}}')
                except:
                    print_exc()
            return text
        # function to create tabs file to be read by server when server restarts or new client connects to server 
    def get_tabs(self):
        ...
        Returns:
            [type]: [description]
        """
        text = self.try_read_file(self.path_tabs)
        ...
        try:
            ...
            print(f"Error occurred at: {e.pos}")
            print(text[e.pos - 10:e.pos + 10])
        
        
    # function to create tabs file to be read by s1erver when server restarts or new client connects to server
    def set_tabs(self, tab):
        """AI is creating summary for set_tabs

        Args:
            symbol ([type]): [description]
        """
        # add tab to dictionary of tabs file if not already in list of tabs
        # add tab to dictionary of tabs file
        try:
            with open(self.path_tabs, 'w') as f:
                try:
                    msvcrt.locking(f.fileno(), msvcrt.LK_LOCK, 1)                    #write array of tab in the file of tabs file and be sure to be in the right format for json 
                    f.write(f'{tab}'.replace("'", "\""))
                except IOError:
                    print("File is used by another program, please try again")
        except:
            print_exc()
    # FUNCTION change all all true to false 
    def MarkAllRead(self):
        text = self.try_read_file(self.path_messages)
        data = json.loads(text)
        for key in data:
          data[key]["isUnRead"] = "false"
        try:
            with open(self.path_messages, 'w') as f:
                try:
                    msvcrt.locking(f.fileno(), msvcrt.LK_LOCK, 1)                    #write array of tab in the file of tabs file and be sure to be in the right format for json 
                    f.write(f'{data}'.replace("'", "\""))
                except IOError:
                    print("File is used by another program, please try again")
        except:
            print_exc()
